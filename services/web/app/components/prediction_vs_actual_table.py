...
def prediction_vs_actual_table(RoundNumber, has_rain):
    ...
    pred_table = df[['FullName', 'TeamName', 'EventFormat', 'Position_Q', 'CleanPredictedPosition', 'ActualPosition']]
    pred_table.columns = ['FullName', 'TeamName','EventFormat', 'GridPosition', 'PredictedPosition',  'ActualPosition']
    return pred_table.sort_values(by=['ActualPosition', 'PredictedPosition']).to_dict('records')
